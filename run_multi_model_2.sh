...
for model in "${model_lst[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=1 bash run.sh $model defect
    # # #you can indeed run defect in main.py(run_gen.py),
    # # # in which 'em' refers to eval_acc * 100, and bleu=codebleu=0

    # # #NLG
    CUDA_VISIBLE_DEVICES=1 bash run.sh $model translate java-cs
    CUDA_VISIBLE_DEVICES=1 bash run.sh $model translate cs-java
    # CUDA_VISIBLE_DEVICES=1 bash run.sh $model refine small
    # CUDA_VISIBLE_DEVICES=1 bash run.sh $model refine medium
    # CUDA_VISIBLE_DEVICES=1 bash run.sh $model generate

    
    # CUDA_VISIBLE_DEVICES=1 bash run.sh $model summarize go
    # CUDA_VISIBLE_DEVICES=1 bash run.sh $model summarize python
    # CUDA_VISIBLE_DEVICES=1 bash run.sh $model summarize java
    # CUDA_VISIBLE_DEVICES=1 bash run.sh $model summarize php
