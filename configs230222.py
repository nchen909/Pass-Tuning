...
def set_hyperparas(args):
    ...
    if args.task == 'summarize':
        ...
        args.lr = 8e-6 if not args.prefix_tuning else 5e-4#8e-6#8e-4
    ...
    if args.few_shot == -1:
        if args.task in ['clone']:
            ...
            args.num_train_epochs = 140 #if not torch.cuda.is_available() else 10*torch.cuda.device_count()//2*2
            # if args.is_clone_sample:
            #     args.num_train_epochs = args.num_train_epochs * 10
            args.patience = args.num_train_epochs*1000#min( 10, args.num_train_epochs//5*5)
            ...
            args.num_train_epochs = 60 if not torch.cuda.is_available() else 60*torch.cuda.device_count()//2#60
            args.patience = min( 10, args.num_train_epochs//5*2)
